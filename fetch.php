...
//  $query = "
//  SELECT students.id,students.fname, students.lname, students.email, students.contactno, students.address, students.dob, students.gender, students.class, students.subject, students.total_fee, fees.amount as Amount, students.status  FROM students left join fees  on students.id=fees.student_id GROUP BY students.id
//  ";
$query = "
...
if($total_data > 0)
{
  foreach($result as $row)
  {
    ...
    <td>'.$row["total_fee"].'</td>
    <td>'.$row["Amount"].'</td>
    <td>'.$row["total_fee"]-$row["Amount"].'</td>
    <td>'.$row["status"].'</td>
    <td>
    <a class="btn btn-success" href="edit.php?id='.$row["id"].'">Update</a>
